...
## 🎯 このプロジェクトを作った理由
このアプリは、**実務レベルの業務アプリ開発力**を示すために設計・実装したフルスタック Web アプリケーションです。  
ユーザー・ホスト・管理者という複数ロールを扱い、**実際の業務で発生しうるユースケースに耐える設計と実装**を意識しています。
...
- **DB:** MySQL / MariaDB
- **テスト:** JUnit5, MockMvc, Mockito
- **外部サービス:** Stripe, Mailgun（想定）
- **デプロイ:** Heroku / Railway
...
---
※ 本ポートフォリオでは、上記「主要機能」に記載したユーザー／ホスト／管理者向けの機能および予約〜決済までの基本業務フローはすべて実装済みです。  
